...
// CORS erlauben für deine HA-URL (später anpassen!)
...
app.post('/chat', async (req, res) => {
  try {
    ...
    const profilText = profileDescriptions[profileId] || "unbekanntes Profil";

    const systemPrompt = `
Du bist ein KUNDE der AOK NordWest in einem Telefontraining.
Sprich NUR als Kunde, niemals als KI.
...
z.B. "Ja hallo, hier ist ${profilText.split(',')[0]}, worum geht es denn?".
    ...
    // Falls noch keine History: Kunde soll anfangen
    if (!history || history.length === 0) {
      messages.push({
        role: 'user',
        content: 'Starte das Gespräch, indem du dich als Kunde meldest.'
      });
    }
